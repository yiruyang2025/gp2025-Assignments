...
int Lasso::strokeAdd(int mouse_x,
    ...
    if (d < 0)//first time
    {
        ...
        if (igl::unproject_onto_mesh(Eigen::Vector2f(x, y),
            ...
            Eigen::Vector3f proj = igl::project(pt.transpose().cast<float>().eval(), modelview, viewer.core().proj,
                                                viewer.core().viewport);
            d = proj[2];
        }
    ...
    pt = igl::unproject(Eigen::Vector3f(x, y, 0.95 * d), modelview, viewer.core().proj,
                        viewer.core().viewport).transpose().cast<double>();
...
bool Lasso::point_in_poly(std::vector<std::vector<unsigned int>> poly, double xt, double yt) {
    Eigen::MatrixXd V;
    igl::list_to_matrix(poly, V);
    Eigen::MatrixXi E(V.rows(), 2);
    for (int v = 0; v < V.rows(); v++) { E.row(v) << v, (v + 1) % V.rows(); }
    return std::abs(igl::winding_number(V, E, Eigen::RowVector2d(xt, yt))) > 0.5;
}
...
void Lasso::strokeFinish(Eigen::VectorXi &selected_vertices) {
    ...
    Eigen::VectorXi is_selected;
    is_selected.setZero(V.rows(), 1);
    ...
    if (Fsel.rows() > 0) {
        ...
        Eigen::VectorXi total;
        total.setZero(ncomp, 1);
